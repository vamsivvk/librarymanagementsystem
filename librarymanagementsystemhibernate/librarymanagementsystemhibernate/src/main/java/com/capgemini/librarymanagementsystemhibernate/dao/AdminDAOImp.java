...
public class AdminDAOImp implements AdminDAO{
	...
	public boolean issueBook(int id , int book_id) {
		...
		try {
			...
			int S = count.size();
			if(S>=1) {
				Query q1 = manager.createQuery("select r from RequestBookBean r where r.id = :id and r.reqBookPK.bid = :bid");
				...
				if(s>=1) {
					...
					if(s1>=1) {
						...
						if(noOfBooksBorrowed<3) {
							...
							Query q5 = manager.createQuery("delete from RequestBookBean r where r.id = :id and r.reqBookPK.bid = :bid");
	...
	public boolean returnBook(int id, int book_id) {
		...
		try {
			...
			transaction = manager.getTransaction();
			transaction.begin();
			Query q = manager.createQuery("select b from BookIssueDetailsBean b where b.id = :id and b.issuePK.bid = :bid");
			q.setParameter("id", id);
			q.setParameter("bid", book_id);
			List count = q.getResultList();
			int i = count.size();
			if(i>=1) {
				Query q1 = manager.createQuery("select r from RequestBookBean r where r.id = :id and r.reqBookPK.bid = :bid and type = :type");
				q1.setParameter("id", id);
				q1.setParameter("bid", book_id);
				...
				if(i1>=1) {
					...
					q2.executeUpdate();
					Query q3 = manager.createQuery("delete from BookIssueDetailsBean b where b.issuePK.bid = :bid and id =:id");
					...
					q4.executeUpdate();
					Query q5 = manager.createQuery("delete from RequestBookBean r where r.id = :id and r.reqBookPK.bid = :bid and type = :type");
					q5.setParameter("id", id);
					q5.setParameter("bid", book_id);
